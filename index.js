...
app.get('/restaurantes',function(request, response) {
 ...
 app.get('/comidas_cliente', function(request, response) {
     pg.connect(process.env.DATABASE_URL, function(err, client, done) {
         client.query("SELECT C.nombre, C.precio, O.id_orden, R.tiempo, R.estado FROM Comida C join comida_pertenece_orden O on O.id_comida = C.id_comida join Orden R on R.id_orden=O.id_orden order by O.id_orden",function(err, result){
           done();
           if(err){
             console.error(err);
             response.send("Error type " + err );
...
app.post('/historialOrdenes', function(req, res) {
        ...
        pg.connect(process.env.DATABASE_URL, function(err, client, done) {
        var sql = { query: 'SELECT R.nom_restaurante, O.id_orden, O.id_restaurante, O.estado, O.ispaid, O.tiempo, O.id_cliente FROM ', table: 'Orden O', join:' inner join Restaurante R on R.id_usuario=O.id_restaurante' , where: ' where O.id_cliente = '+"\'"+req.body.id_cliente+"\'", order: ' order by O.tiempo desc'};
